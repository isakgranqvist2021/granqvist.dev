"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.GRID_ITEMS = void 0;
const images_1 = require("./images");
exports.GRID_ITEMS = [
    {
        src: images_1.images['go'],
        backgroundSize: '60%',
        alt: 'go programming language',
    },
    {
        src: images_1.images['node'],
        backgroundSize: '55%',
        alt: 'nodejs javascript runtime',
    },
    {
        src: images_1.images['react'],
        backgroundSize: '40%',
        alt: 'react javascript library by facebook',
    },
    {
        src: images_1.images['angular'],
        backgroundSize: '30%',
        alt: 'angular javascript framework',
    },
    {
        src: images_1.images['typescript'],
        backgroundSize: '70%',
        alt: 'typescript javascript with type safety',
    },
    {
        src: images_1.images['gcp'],
        backgroundSize: '20%',
        alt: 'google cloud platform cloud service provider',
    },
];
